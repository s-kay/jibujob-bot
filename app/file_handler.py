...
async def generate_and_upload_pdf(cv_data: dict, desired_filename: str) -> str | None:
    ...
    try:
        doc = SimpleDocTemplate(temp_filename, pagesize=letter, leftMargin=0.75, rightMargin=0.75, topMargin=0.75, bottomMargin=0.75)
    ...
    try:
        async with httpx.AsyncClient() as client:
            async with aiofiles.open(local_filepath, "rb") as f:
                ...
                # This service expects the file in a specific form field name: 'files[]'
                files = {'files[]': (desired_filename, content_bytes, 'application/pdf')}
                ...
                # This service returns the link directly in the response body
                download_link = response.text.strip()
                if not download_link.startswith("http"):
                    raise ValueError(f"Upload service returned an invalid link: {download_link}")
