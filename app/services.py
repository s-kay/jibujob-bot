    ...
    if message_text == "1" or session.current_menu == "jobs":
        ...
        if message_text == "7":
            if not session.resume_data or not session.resume_data.get('full_name'):
                ...
                await whatsapp_client.send_whatsapp_message(session.phone_number, reply)
                session.current_menu = "main" # Reset menu to avoid getting stuck
                return

            session.cover_letter_data = {}; reset_flags(); message_text = ""
        ...
        if is_complete:
            # AI Generation happens here, as the conversation part is done
